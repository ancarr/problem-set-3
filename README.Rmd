...
cleaned_data <- raw_data %>% mutate(.id = row_number()) %>% gather(key, value, -.id, -gene.name) %>% separate(key, c("plant", "fungus", "time.point", "rep"), "_") 
...
cleaned_data %>% filter(time.point == "6hpi") %>% arrange(desc(value)) %>% head(n = 1) %>% select(plant) 
...
cleaned_data %>% filter(time.point == "18hpi") %>% arrange(desc(value)) %>% head(n = 1) %>% select(plant, fungus)
...
cleaned_data_time <- cleaned_data_hpi %>% select(-time.point, -time.value) %>% spread(time.hpi, value) %>%
  mutate(expr.diff = hpi.24 - hpi.6) %>% group_by(gene.name, plant, fungus) %>% 
  summarise(mean =  mean(abs(expr.diff)), sd = sd(abs(expr.diff))) %>% group_by(plant, fungus) %>%
  arrange(desc(mean), sd) %>% group_by(plant, fungus) %>% slice(1:3)

final_data <- cleaned_data_time
...
`final_data` contains `r nrow(final_data)` rows of data and `r ncol(final_data)` columns of data.  Interestingly, `r nrow(final_data) + 1` is equal to a number that can be lucky or unlucky. The names of the columns `r colnames(final_data)` are not capitalized thank goodness. 
...
```{r problem_5, include = TRUE, fig.width=8, fig.height=6}

graph_dataset <- cleaned_data_hpi %>% filter(gene.name %in% cleaned_data_time$gene.name)

expression_plot <- graph_dataset %>% ggplot(aes(x = time.value, y = value)) + 
  geom_point(aes(x = time.value, y = value, color = gene.name, shape = rep)) + facet_wrap(plant ~fungus) +
  geom_smooth(se = FALSE, aes(group = gene.name)) + theme_bw() + ggtitle("Differential Gene Expression Over
  Time by Plant species and Fungus")

#Can't get the plot to display in the markdown document
